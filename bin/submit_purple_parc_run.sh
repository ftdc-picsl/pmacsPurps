...
for x in `cat $filelist `; do 
    ...
    if [[ ! -f $purpseg ]] ; then
        ...
        cmd="${bindir}/run_parc_run.sh ${i} ${hemi} ${purplerepo} ${bidsBase} ${outBase} ${freeoutdir} ${n_threads} "
        echo $cmd 
        # bsub -N -J ${filestem}_purplefree -o ${logdir}/${filestem}_purplefree_log_%J.txt -n 1 -M 32GB $cmd
    fi
