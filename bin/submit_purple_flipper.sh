...
for x in `cat $filelist `; do 
    ...
    if [[ ! -f $purpseg ]] ; then
        ...
        cmd="${bindir}/run_flipper.sh ${i} ${hemi} ${purplerepo} ${bidsBase} ${outBase} ${freeoutdir} ${n_threads} "
        echo $cmd 
        ocmd="bsub -q $queue -N -J ${filestem}_purplefree -o ${logdir}/${filestem}_purplefree_log_%J.txt -n 2 -M 16GB $cmd "
	echo $ocmd
	$ocmd
	sleep .25s
    fi
